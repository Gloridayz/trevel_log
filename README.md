...
PIN 입장 → **카테고리(여행) 선택** 화면 → 카테고리 하나를 고르면 그 안에서 탭 5개를 씁니다. 카테고리는 관리자만 추가/이름변경/삭제할 수 있습니다 (예: "26년 대만여행", "일상").

1. **기록** — 작성 전용 화면 (관리자만 작성 가능, 폼만 노출)
2. **추억** — 기록 탭에 쌓인 내용을 보기 전용으로 열람 (누구나 조회 가능, 작성 폼 없음)
3. **위시리스트·숙소** — 한 곳씩 입력 또는 여러 곳 한번에 붙여넣기, 구글맵/장소명으로 자동 채우기 지원
4. **경비** — 날짜별 입력, 항목은 드롭다운으로 관리(재사용 가능한 이름표) + 건별 상세 메모, 총합계
5. **준비물** — 담당(챙기는 사람)별 체크리스트, "남은 것만 보기" 필터 + 남은 개수 표시
...
- **예전 버전(카테고리가 자유 텍스트였던 버전)을 이미 쓰고 있던 경우**: `supabase/migrate_002_trip_categories.sql`을 대신 실행하세요. 기존 기록/위시리스트/경비/준비물 데이터를 지우지 않고 새 구조(카테고리 선택 화면)로 옮겨줍니다. 실행 전 Supabase 대시보드의 백업 상태를 한 번 확인해두는 걸 권장합니다.
